...
enum en_t { UNIFORM_SPACED, CHORD_LENGTH };
enum en_knot { k_SPACED, AVERAGE, UNIVER };
...
const double Nikt(int i, int k, double t) {
  ...
  if (abs(d1) < 1e-6)
    ...
    a = (t - x[i]) * Nikt(i, k - 1, t) / d1;
  if (abs(d2) < 1e-6 || i + k >= x.size())
    b = 0;
  else
    b = (x[i + k] - t) * Nikt(i + 1, k - 1, t) / d2;
  return a + b;
}
...
get parm t
*/
void generatePram(const vector<Vector3d> *p, vector<float> &param_t, int n,
                  en_t type, int a, int b) {
  param_t.clear();
  double length = 0.0;
  double ct = 0;
  vector<Vector3d> points;
...
void calColpoints(vector<Vector3d> &points, int order, vector<float> &knot,
                  vector<float> &param, int n, MatrixXd &P) {
  MatrixXd N(n + 1, n + 1);
  for (int i = 0; i <= n; ++i) {
    for (int j = 0; j <= n; ++j) {
...
void calculate_Q(MatrixXd &D, MatrixXd &P, MatrixXd &Qk, MatrixXd &Q,
                 MatrixXd &N, vector<float> &knot, vector<float> &param_t,
                 int order) {
  int n = D.rows() - 1;
  int h = P.rows() - 1;
  for (int k = 0; k < 3; ++k) {
  ...
  for (int k = 1; k < n; ++k) {
    for (int i = 0; i < 3; ++i) {
      Qk(k - 1, i) = D(k, i) - cal_N(knot, 0, order, param_t[k]) * P(0, i) -
                     cal_N(knot, h, order, param_t[k]) * P(h, i);
    }
  }
  for (int i = 0; i < h - 1; ++i) {
    for (int k = 0; k < 3; ++k) {
      double sum = 0.0f;
      for (int j = 1; j < n; ++j) {
        sum += cal_N(knot, i, order, param_t[j]) * Qk(j - 1, k);
      }
      Q(i, k) = sum;
    }
  }
  for (int k = 1; k < n; ++k) {
    for (int i = 0; i < h - 1; ++i) {
      N(k - 1, i) = cal_N(knot, i + 1, order, param_t[k]);
    }
  }
...
void curve_approximation(Curve &cv) {
  ...
  calculate_Q(D, P, Qk, Q, N, cv.knot, cv.param_t, cv.order);
  cout << D << endl;
  cv.D = D;
  cv.P = P;
}

void surface_approximation(Surface &s) {
  ...
  for (int i = 0; i <= col; ++i) {
    ...
    MatrixXd D(row + 1, 3);
    for (int k = 0; k <= row; ++k) {
      points.push_back(Vector3d(s.points[k][i])); // column d
      for (int c = 0; c < 3; ++c) {
        D(k, c) = s.points[k][i][c];
      }
    }
    MatrixXd Q(s.e - 1, 3), P(s.e + 1, 3), Qk(row - 1, 3), N(row - 1, s.e - 1);
    calculate_Q(D, P, Qk, Q, N, s.knot_U, s.param_s, s.order_p);
    for (int k = 0; k <= s.e; ++k) {
      s.Q[i].push_back(Vector3d(P(k, 0), P(k, 1), P(k, 2)));
    }
  }
  row = s.e;
  s.P.resize(row + 1);
  for (int c = 0; c <= row; ++c) {
    ...
    MatrixXd Q(s.f - 1, 3), P(s.f + 1, 3), Qk(col - 1, 3), N(col - 1, s.f - 1);
    calculate_Q(D, P, Qk, Q, N, s.knot_V, s.param_t, s.order_q);
    for (int k = 0; k <= s.f; ++k) {
...
void global_curve_approximation() {
  ...
  generatePram(&curve.points, curve.param_t, curve.n, en_t::UNIFORM_SPACED, 0,
               1);
  generateKnot(curve.knot, en_knot::k_SPACED, curve.h, curve.degree,
               curve.param_t); // knot size h + 1
...
void global_surface_approximation() {
  command = 4;
  readSurdata(
      surface.points,
  ...
  if (surface.e >= surface.m) {
    fprintf(stderr, "ERROR: m > e\n");
    exit(1);
  }
  if (surface.f >= surface.n) {
    fprintf(stderr, "ERROR: n > f\n");
    exit(1);
  }
  if (surface.e < surface.p) {
    fprintf(stderr, "ERROR: e >= p\n");
    exit(1);
  }
  if (surface.f < surface.q) {
    fprintf(stderr, "ERROR: f >= q\n");
    exit(1);
  }

  generatePram(nullptr, surface.param_s, surface.m, en_t::UNIFORM_SPACED, 0, 1);
  generateKnot(surface.knot_U, en_knot::k_SPACED, surface.e, surface.p,
               surface.param_s);

  generatePram(nullptr, surface.param_t, surface.n, en_t::UNIFORM_SPACED, 0, 1);
  generateKnot(surface.knot_V, en_knot::k_SPACED, surface.f, surface.q,
               surface.param_t);
...
void globel_surface_interpolation() {
  ...
  generatePram(nullptr, surface.param_s, surface.m, en_t::UNIFORM_SPACED, 0, 1);
  generateKnot(surface.knot_U, en_knot::k_SPACED, surface.m, surface.p,
               surface.param_s);

  generatePram(nullptr, surface.param_t, surface.n, en_t::UNIFORM_SPACED, 0, 1);
  generateKnot(surface.knot_V, en_knot::k_SPACED, surface.n, surface.q,
               surface.param_t);
  inter_Q();
}
